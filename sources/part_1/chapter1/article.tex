...
\begin{reviewread}
Hospitals produce millions of clinical documents every year. These could be used to train language models internally, but the resulting models inherit the legal constraints of their training data: they cannot be released publicly or exchanged between institutions. For a model that is truly open, the training data must come from somewhere else.
\end{reviewread}

\begin{reviewread}
Yet hospital records remain the richest source of medical information, since much of it appears only in free text. In two clinical-trial recruitment settings, unstructured narratives were needed to resolve 59\% and 77\% of the eligibility criteria~\citep{raghavan_how_2014}. Clinical data warehouses have made these records increasingly accessible for research purposes. Extracting this information, however, requires high-performing language models adapted to the biomedical domain.
\end{reviewread}
...
\begin{reviewread}
\citet{copara_contextualized_2020} explored a second phase of pre-training on 31,000 French biomedical scientific articles. However, they did not observe a significant improvement on a clinical named entity recognition task using the large version of CamemBERT. This could be explained by the combination of a relatively small corpus (31K documents compared to the 18 billion words in BioBERT~\citep{lee_biobert_2019}) and the large version of the CamemBERT model.
\end{reviewread}
...
\begin{reviewread}
Third, all documents from the selected sources were included indiscriminately, without assessing whether individual passages were genuinely useful for pretraining. Whether such filtering would improve downstream performance is an open question that we explore in \Cref{chap:quality,chap:mcbio}.
\end{reviewread}
