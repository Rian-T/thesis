...
\paragraph{LLM quality annotation.} The same LLM annotates each passage along four quality signals scored from 1 to 10, listed in \Cref{tab:quality-signals} with the abbreviations used in the ablation tables. A content type is also assigned among nine categories: \texttt{research\_\allowbreak findings} (526k passages), \texttt{medical\_\allowbreak knowledge} (388k), \texttt{clinical\_\allowbreak guidance} (218k), \texttt{research\_\allowbreak methodology}, \texttt{drug\_\allowbreak information}, \texttt{background\_\allowbreak review}, \texttt{policy\_\allowbreak administrative}, \texttt{patient\_\allowbreak case} (36k), and \texttt{other}. In total, 2.16 million passages are annotated.
...
\begin{tabular}{llc}
\toprule
\textbf{Quality signal} & \textbf{Code} & \textbf{Mean} \\
\midrule
Educational value & \texttt{edu} & 6.5 \\
Content richness & \texttt{cont} & 6.8 \\
Writing quality & \texttt{writ} & 7.6 \\
Terminological precision & \texttt{term} & 7.6 \\
\bottomrule
\end{tabular}
\caption{The four quality signals annotated on each passage, with the code used in the ablation tables and the mean score over the 2.16 million annotated passages on the 1--10 scale.}
\label{tab:quality-signals}
\end{table}
...
\begin{reviewread}
We measure the effect of each content type by ablation: we remove one type and compare against the full corpus on three evaluation tasks (DiaMed~\citep{drbenchmark2024}, FrACCO-30~\citep{pignat2024fracco}, and EMEA~\citep{neveol14quaero}, 9 seeds). \Cref{tab:mcbio-content-type} shows that removing \texttt{drug\_information} \emph{improves} performance by $+0.67$pp, while removing \texttt{medical\_knowledge} degrades it by $-0.13$pp. A random control (removing the same amount of text at random) gives $+0.04$pp, confirming that the effect is specific to the content type.
\end{reviewread}
...
Random removal (same volume) & 63.72 & $+$0.04pp \\
...
\begin{reviewread}
We exclude \texttt{drug\_information} from the final corpus (redundant with the EMEA source already included separately) and \texttt{other}. Paragraphs shorter than 50 tokens, roughly 35 words, are also excluded.
\end{reviewread}
...
\begin{reviewread}
Two findings emerge from the ablations. First, not all content types contribute equally: removing drug information actually improves downstream performance, while removing medical knowledge degrades it. Second, not all quality signals work as filters: educational value and content richness help, but writing-quality and terminological-precision filters \emph{hurt} performance, likely because they remove the informal but informative clinical documents that ultimately matter most. The lesson, perhaps, is that curation should be guided by what is being said rather than by how well it is written.
\end{reviewread}
