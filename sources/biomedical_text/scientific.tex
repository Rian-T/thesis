...
sentences are complete, each one follows from the one before, and the passage even
pauses to name what is still unknown and to promise it will study it. There is also
a great deal of writing like this, and unlike the clinical note it is free to read.
It is tempting to treat it as the clinical language after all, just written with
more care and easy to find. It is not. The research article is abundant and public,
but it is a language of its own.

% === P2 ===
The two texts describe the same disease, so the article can look like a tidier
version of the note. What separates them is not their care or their subject but
their purpose. The rhetorician Carolyn Miller asked why texts settle into the forms
they do, and answered that a genre is not a shape on the page but a recurring action
in a community: to know a genre is to know what it is for and what it does
\citep{miller_genre_1984}. By that test the two are different genres because they
answer different needs. The clinical note exists to record a patient and guide their
care. The research article exists to put a claim before the field and open it to
challenge. That difference in purpose, not a difference in polish, shapes how each
one is written.

% === P3 ===
The article's purpose shows in its shape. The linguist John Swales studied how
researchers open their papers and found a pattern so regular that he named it:
first stake out a territory, then carve a niche by showing what is missing from it,
then occupy that niche with the present work \citep{swales_genre_1990}. The example
...
understood}; and it occupies that niche by pointing to the body of work, its own
...
question, the procedure used to answer it, what was observed, and what it means.
The arrangement feels timeless, but it is recent. In a survey of 1{,}297 original
articles from four leading medical journals between 1935 and 1985, not one used this
structure in 1935; by the 1970s more than four in five did, and by the 1980s it was
the only accepted form for an original paper \citep{sollaci_imrad_2004}. A format
only a few decades old now carries the shape of scientific reasoning in its headings
alone, so that a reader can jump to the question, the method, or the verdict
...
studies of academic writing call this running layer metadiscourse: the part of the
writing that is not the claim itself but the author's handling of it, including the
words that tune how strongly the claim is made
\citep{hyland_metadiscourse_2005, hyland_tse_2004}. Hedges such as \emph{may} or
\emph{suggests} hold a statement back, while boosters such as \emph{clearly} or
\emph{shows} push it forward. \emph{These results suggest that X may contribute to
Y} is a claim offered for debate, while the plain \emph{X causes Y} would be far
bolder. The note hedges too, but rarely and for another reason: its \emph{rule out
myocardial infarction} is an instruction to act, not a position staked out in an
argument.

% === P6b — Biber quantitative register ===
These impressions can be counted. The linguist Douglas Biber measured which
features cluster together across many spoken and written texts and placed scientific
...
\citep{biber_variation_1988}. The density is not even uniform within one article.
...
one citation to the next: \emph{X may contribute to Y} becomes, once enough people
repeat it unchallenged, simply \emph{X causes Y} \citep{latour_laboratory_1979}. The
hedges that fill the literature are therefore the visible trace of claims still
being argued over, and a model that flattens them mistakes a provisional finding for
a settled fact.

% === P8 ===
There is a more formal way to say all of this. A sublanguage, in the sense the last
chapter borrowed from Zellig Harris, is the restricted grammar a specialised domain
...
from biology articles, and found they were genuinely distinct: each domain needs its
...
medicine, not to record a patient. More articles, on their own, do not bring us
