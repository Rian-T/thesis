...
% thesis-ified: hook intro, \section*{Conclusion} on insight, RW deferred to \Cref{chap:rw-corpus/lm/ie}, cross-refs added; tables on booktabs+ThesisTableSep. (remaining: trim multi-sentence captions in the global style sweep)
...
\begin{tikzpicture}[scale=0.78, transform shape,
  ...
  frame/.style={rounded corners=5pt, draw=ThesisNeutral!40, minimum height=3.2cm},
]

  % === THREE FRAMES ===
  \node[frame, minimum width=2.5cm]  (f1) at (1.25, 1.0) {};
  \node[frame, minimum width=5.0cm]  (f2) at (7.4,  1.0) {};
  \node[frame, minimum width=4.5cm]  (f3) at (14.55,1.0) {};
  ...
  \node[draw=ThesisTertiaryDark, fill=ThesisTertiary!15, rounded corners=2pt,
        text width=3.6cm, font=\scriptsize, inner sep=4pt] (mlmex) at ([yshift=7mm]f3.center)
    {$\mathcal{L}_{\text{MLM}}$:\; Le \colorbox{ThesisTertiary!30}{[MASK]} (E11) peut
     \colorbox{ThesisTertiary!30}{[MASK]} une n\'ephropathie\ldots};
  \node[draw=ThesisTertiaryDark, fill=ThesisTertiary!15, rounded corners=2pt,
        text width=3.6cm, font=\scriptsize, inner sep=4pt] (relex) at ([yshift=-8.5mm]f3.center)
...
To assign a code to a discharge summary, a hospital coder relies on facts that the summary never states. Type 2 diabetes, coded E11, can cause a specific kidney disease coded N08.3. Two diagnoses may be mutually exclusive, so coding one rules out the other. One code sits several levels below another in a strict hierarchy. This knowledge lives in medical ontologies, which record codes and the relations between them as a graph. It does not appear in running text.

The previous chapters improved biomedical encoders in two ways: by collecting and filtering a better pretraining corpus (\Cref{chap:collecting}) and by changing the pretraining objective (\Cref{chap:objectives}). Both operate on running text. An encoder trained this way learns medical vocabulary, but it never sees the relational structure of an ontology, because that structure is not written in sentences. Models such as CamemBERT-bio (\Cref{chap:encoders}) learn what a term means; they do not learn how its code relates to the others.

This chapter asks whether that structure can be turned into a training signal for encoders. Medical coding makes the question concrete. French hospitals code tens of millions of stays a year~\citep{atih_pmsi} with the CIM-10, CCAM, and ATC classifications, and accurate coding depends on the relations between codes.
We turn those relations into text. We walk the ontology graph to produce sequences of related codes, rewrite each walk into fluent textbook prose with a large language model, and train the encoder on this text with two objectives at once: masked language modeling and relation prediction between pairs of codes. The single condition that makes this work is that both objectives read the same text. We call this property \emph{alignment}, and we show it is necessary.

We evaluate \method on three French medical coding benchmarks, study the contribution of each component, and release the 1.3 million reformulated textbooks across the three ontologies (CIM-10, CCAM, ATC) together with the pretrained checkpoints.
\section{Related Work}
\label{sec:related}

Two lines of work meet in this chapter: methods that bring ontology structure into language models, and automatic medical coding, the task we evaluate on. \Cref{chap:rw-corpus} reviews the first at length, so here we recall only what is needed and focus on the point of contact with our method.

\subsection{Bringing Ontology Structure into Language Models}

Biomedical encoders such as BioBERT \citep{lee_biobert_2019}, PubMedBERT \citep{gu_domain-specific_2022}, and CamemBERT-bio \citep{touchent2023camembertbio} learn medical vocabulary from large text corpora (\Cref{chap:rw-lm}). They do not capture the relational structure of medical ontologies, since that structure is not expressed in running text. Two families of methods try to close this gap, both described in \Cref{chap:rw-corpus}. Static graph embeddings such as RDF2Vec and Snomed2Vec run random walks over an ontology and embed the resulting sequences, but they produce fixed vectors that cannot be used for transformer pretraining. Knowledge-enhanced transformers such as KEPLER and DRAGON inject graph structure into pretraining, but they rely on text that is already aligned with a graph rather than generating training text from the graph alone. The idea of generating synthetic textbook-quality data with a language model comes from the Phi series \citep{gunasekar2023textbooks} and, for knowledge sources, EntiGraph \citep{yang2025entigraph}.

Closest to the present work, BioOntoBERT \citep{shashikumar2023bioontobert} generates sentences from biomedical ontologies using Onto2Sen templates and pretrains BERT with masked language modeling on the resulting corpus. The template-based generation produces formulaic sentences (e.g., ``X is a type of Y'') rather than fluent prose, and the method uses only masked language modeling without relation prediction. \Cref{tab:related_comparison} summarizes how \method combines components not jointly present in prior work.
...
\bottomrule
\end{tabular}
\caption{\textbf{Comparison with related approaches.} \method is the only listed approach that combines the three components for biomedical encoder pretraining.}
\label{tab:related_comparison}
\end{table}

\subsection{Automatic Medical Coding}

Automatic medical coding assigns diagnosis, procedure, or medication codes to clinical text. \Cref{chap:rw-ie} reviews the main approaches, from label-wise attention models such as CAML \citep{mullenbach2018caml} to fine-tuned encoders such as PLM-ICD \citep{huang2022plmicd}. \citet{kirchler2026grasp} recently showed that language-model embeddings improve the transfer of EHR-based predictions across countries and coding systems, which points to the value of encoding medical knowledge structure for cross-system generalization. These methods improve the classification head or the input representation, but they rely on generic encoders pretrained on flat text. Our contribution sits one step earlier, at pretraining: an encoder that has already learned ontology structure before it sees any coding data.

\section{Method}

\noindent The walk, generation and training settings behind this section are given in \Cref{app:ontobook-details}.
\label{sec:method}
...
\begin{reviewread}
We prompt Qwen3-235B-A22B-Instruct \citep{yang2025qwen3} with FP8 quantization to transform each walk into a coherent paragraph. The prompt instructs the model to: (1) preserve all medical codes and their relationships exactly as stated, (2) use natural medical language without lists or bullet points, (3) not add any information beyond what appears in the walk. The prompt is designed to keep the textbook content grounded in the ontology structure. We apply two filters: we discard reformulations shorter than 50 characters or that fail to mention the source codes. The reformulation runs on 4 NVIDIA H100 GPUs using vLLM \citep{kwon2023vllm} with guided decoding to ensure valid JSON output, taking approximately 20 hours for all 1.3 million walks. The resulting textbooks contain approximately 1.3 billion tokens across all three ontologies. Figure~\ref{fig:textbook_example} shows an example transformation.
\end{reviewread}
...
\end{tikzpicture}
\caption{\textbf{Walk-to-textbook transformation.} The structured walk is reformulated into fluent medical prose with instructions to preserve all codes and relations.}
...
The model optimizes two objectives jointly. The first, $\mathcal{L}_{\text{MLM}}$, follows standard masked language modeling \citep{devlin_bert_2019}: we mask 15\% of tokens from textbook paragraphs and train to predict them. The second, $\mathcal{L}_{\text{rel}}$, classifies the relationship between pairs of medical codes. Our main model uses CIM-10; we explore transfer from CCAM and ATC in Section~\ref{sec:multi_onto}. We extract 282,907 code pairs from the CIM-10 ontology across 6 relation types: \textsc{Parent}, \textsc{Child}, \textsc{Sibling}, \textsc{Differential Diagnosis}, \textsc{Causes}, and \textsc{Caused\_By}. For each pair, we retrieve the corresponding textbook descriptions generated in Section~\ref{sec:textbook}. The input format is "$[\texttt{CLS}]~\text{text}_A~[\texttt{SEP}]~\text{text}_B~[\texttt{SEP}]$", where $\text{text}_A$ and $\text{text}_B$ are the textbook descriptions of the two codes, and a separate linear classification head on the $[\texttt{CLS}]$ token predicts the relation type, trained with cross-entropy loss over the 6 classes.
...
\Cref{tab:probing} reports the three probes.
...
Ontology structure never appears in clinical text, yet a coder needs it to assign the right code. Writing that structure out as textbook prose lets an encoder read it, and one condition decides whether the signal survives training: masked language modeling and relation prediction must run on the same text. When they do, the encoder improves on every French coding benchmark, by +2.5 micro-$F_1$ on FRACCO and +8.0 on Distemist over the same model trained with masked language modeling alone. When they run on different text, the two objectives pull against each other and performance drops by 30 points. Alignment is what separates a knowledge-grounded encoder from one that has only seen more text.

\vspace{2em}

The gains of the first two parts were all measured on benchmarks whose label set was
fixed in advance, which is not how the task that motivates this thesis arrives.
\Cref{part:adaptation} takes a pretrained encoder to that task and asks what
fine-tuning it there actually requires.
