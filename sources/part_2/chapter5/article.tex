...
Masked language modeling sees only 15\% of its input. At every training step, 85\% of the tokens receive no gradient signal. Causal language modeling instead predicts every token from the tokens before it, so every position contributes a gradient signal, at the cost of only ever looking backward. The question is whether an encoder can benefit from this dense training signal without permanently losing its ability to look in both directions.
...
\subsection{When Decoder Continue-Pretraining Stops Working}
\label{sec:decoder-cpt-stops}

% TODO: Write content. Key points to cover:
%   - BioMistral -0.9 pts after CPT (recovered only via model merging)
%   - Meditron-70B: 128 A100 GPUs for 332 hours, +1.8 pts on biomed benchmarks
%   - Dorfner et al. 2024: OpenBioLLM-8B 30% vs Llama-3-8B 64% on NEJM
%   - Forgetting: CPT destroys instruct capabilities (catastrophic forgetting of
%     instruction following, not classical knowledge forgetting)
%   - Data overlap: LLMs have already seen PubMed/PMC during initial pretraining,
%     so CPT shows them data they already know — explains the diminishing returns
%   - Compute-performance trade-off: encoders give more F1 for less compute
%   - Why encoders still matter for clinical IE (Lehman et al. 2023):
%     deployment constraints, confidentiality, smaller models inside hospitals
%   - Conclusion: this asymmetry between encoders and decoders motivates building
%     better encoders, not forcing decoders to specialize
