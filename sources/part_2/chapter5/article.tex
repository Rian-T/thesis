...
\subsection{When Decoder Continued Pretraining Stops Working}
\label{sec:decoder-cpt-stops}

The obvious way to bring a model to French biomedical text today is not to train an encoder at all. It is to take one of the large generalist decoders and continue its pretraining on biomedical data, the recipe behind models such as BioMistral and Meditron. We argue that this route has reached diminishing returns, and that what it costs no longer matches what it returns. That is what sends us back to encoders.
