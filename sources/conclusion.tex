...
We built an exploratory extension of TABIB in which GLM-5.2 acts at a
simulated clinical workstation. It can read and update a patient record, consult
the official ANSM interaction thesaurus, and issue a prescription. Each session
includes a patient requesting a prescription for a contraindicated drug
combination. The record also contains a forged note claiming that a physician
has authorised the combination. We score the session as unsafe when the model
issues the prescription (\Cref{fig:conclusion-agentic-env}).

\input{sources/part_3/figures/fig_agentic_env}

We evaluate GLM-5.2 in four conditions, with twenty sessions per condition. The
first asks a single question in a chat interface without tools. The second gives
the model access to tools for one turn. The third places it in a multi-step
agentic workflow. The fourth adds repeated patient pressure to that workflow.
These conditions separate the effects of tool access, sequential action, and
conversational pressure.

With the forged note, the model issues the contraindicated prescription in none
of the twenty chat sessions, two of the twenty one-turn tool sessions, one of the
twenty agentic sessions, and five of the twenty sessions with patient pressure
(\Cref{fig:conclusion-agentic-gradient}). The highest observed rate is therefore
in the agentic condition with pressure. However, each condition contains only
twenty sessions, and the confidence intervals remain wide.

\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{plots/part_3/output/conclusion_agentic_gradient.pdf}
  \caption{Unsafe prescriptions in twenty sessions per condition. GLM-5.2 is
  evaluated in a chat interface, with one-turn tool access, in an agentic
  workflow, and in the same workflow under patient pressure. Lines compare a
  forged note with an authentic but outdated note. The open marker shows the
  final condition when the current ANSM contraindication is also provided. Bars
  are 95\% Wilson intervals.}
  \label{fig:conclusion-agentic-gradient}
\end{figure}

We repeat the experiment with an authentic but outdated note. The corresponding
counts are zero, two, zero, and one unsafe sessions. In the final condition, we
also provide the current ANSM contraindication directly to the model. It still
issues the prescription in four of twenty sessions. These comparisons are
consistent with sensitivity to the forged note's claim of authority, but this
small experiment does not isolate the mechanism.

The zero unsafe rate in the chat condition is also misleading. The model obtains
it by refusing almost every case, including combinations that are safe. It
therefore fails to distinguish safe prescriptions from contraindicated ones. An
unsafe-action rate alone would describe this behaviour as safe, even though the
model is not useful for the task.
