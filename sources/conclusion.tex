...
This is one model, one scenario, and twenty pairs, with intervals too wide to size
the effect. What the probe points at does not depend on that size. A chat benchmark
has no place to put a failure of this kind: it offers one question and one answer,
and the failure needs a horizon to appear in. A whole class of failure therefore
sits outside what the field's instruments can register, and a benchmark can report
a model safe without having looked where the danger is.

The systems reaching French clinical practice are becoming agents, acting over
several steps on a record, while the evidence offered for them stays a table of
question-answering scores. The distance between what is deployed and what is
measured grows with every deployment.

The same shift closes the obvious way out. Records cannot leave the hospital, so
the evaluation cannot be run outside it. Inside, it is arguably worse. An agent
under evaluation is an agent acting, and learning whether it issues a dangerous
order means letting it reach a real record and issue one; the prescription tool
that this simulation logs and scores would be a real prescription. The test is the
hazard, which a question never was. Nor would it scale, since data cannot be pooled
and neither can the evaluation, so each institution would rebuild the same
apparatus behind its own walls and produce nothing the next one could use. That
leaves one route, the one this thesis took for its corpora and its models and which
its evaluation now needs in turn: rebuild the clinic from public and synthetic
material, well enough that a model cannot tell the reconstruction from the real
thing. Building it is what remains. Only then does the question a hospital actually
has, whether an agent can be trusted to act on a patient's record, become one the
field can ask.
