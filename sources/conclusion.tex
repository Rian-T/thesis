...
A computer that could weigh a patient's signs toward a likely disease: nobody finds
that idea strange now. The systems being installed do it, and they act on the
judgement as well. Medicine has used automated systems for decades, and that work
was done seriously. What has changed is the scale of what stands behind them. The
money is of another order, and a hospital that installs one of these systems is hard
to move off it. Arriving first therefore holds the ground, so speed is what the
market rewards and safety is what gets checked afterwards. Responsibility for that
rests with the firms setting the pace. Research has a different duty: it decides
what a safety claim can be measured with, and for a long time it has accepted a few
points on a benchmark. The patients now being handed these systems are covered by
that measure, and it cannot see the failure this chapter found.
