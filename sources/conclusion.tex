...
evaluation is an agent that acts, and the only way to learn whether it issues a
dangerous order is to let it issue one. The prescription that this simulation writes
to a log would be a real prescription there. It would not scale either, because data
...
Ledley and Lusted wanted a computer that could weigh a patient's signs toward a
likely disease. Nearly seventy years later it is being installed, and it no longer
weighs signs; it acts. This is what the field has worked toward since 1956, and it
has arrived as a market, where the pace is set by the race to deploy rather than by
the evidence that deploying is safe. The firms that build these systems carry the
first responsibility for what they do. Research carries the rest, and no one else
will take it. The models are reaching patients now, and what they do there still has
to be measured. That is the work that remains.
